import 'package:structured_pinterest/utils/constants/image_constatnts.dart';

class Dummydb {
  //detailing page data
  static List<Map> Dataimage = [
    {
      'imagepath': ImageConstatnts.image_1,
      'description': "Embracing the tranquility of nature.",
      'profile_pic_url': ImageConstatnts.profile_pic0,
      'name': 'Aisha',
      'followers': '500k followers',
      'likeCount': 3
    },
    {
      'imagepath': ImageConstatnts.image_2,
      'description': "Where the wild things are.",
      'profile_pic_url': ImageConstatnts.profile_pic1,
      'name': 'Carlos',
      'followers': '750k followers',
      'likeCount': 4
    },
    {
      'imagepath': ImageConstatnts.image_3,
      'description': "Nature's masterpiece.",
      'profile_pic_url': ImageConstatnts.profile_pic2,
      'name': 'Emma',
      'likeCount': 10,
      'followers': '1.2m followers'
    },
    {
      'imagepath': ImageConstatnts.image_4,
      'description': "Lost in the beauty of the great outdoors.",
      'profile_pic_url': ImageConstatnts.profile_pic3,
      'name': 'Li Wei',
      'likeCount': 100,
      'followers': '900k followers'
    },
    {
      'imagepath': ImageConstatnts.image_5,
      'description': "Breathe in the fresh air.",
      'profile_pic_url': ImageConstatnts.profile_pic4,
      'name': 'Sophia',
      'likeCount': 67,
      'followers': '1.5m followers'
    },
    {
      'imagepath': ImageConstatnts.image_6,
      'description': "Finding peace in the wilderness.",
      'profile_pic_url': ImageConstatnts.profile_pic5,
      'name': 'Mohammed',
      'likeCount': 40,
      'followers': '2m followers'
    },
    {
      'imagepath': ImageConstatnts.image_7,
      'description': "Wander often, wonder always.",
      'profile_pic_url': ImageConstatnts.profile_pic6,
      'name': 'Olivia',
      'likeCount': 50,
      'followers': '800k followers'
    },
    {
      'imagepath': ImageConstatnts.image_8,
      'description': "Nature does not hurry, yet everything is accomplished.",
      'profile_pic_url': ImageConstatnts.profile_pic7,
      'name': 'Juan',
      'likeCount': 10,
      'followers': '600k followers'
    },
    {
      'imagepath': ImageConstatnts.image_9,
      'description': "Let nature be your guide.",
      'profile_pic_url': ImageConstatnts.profile_pic8,
      'name': 'Aria',
      'likeCount': 19,
      'followers': '1.1m followers'
    },
    {
      'imagepath': ImageConstatnts.image_10,
      'description': "Chasing sunsets and dreams.",
      'profile_pic_url': ImageConstatnts.profile_pic9,
      'name': 'Daniel',
      'likeCount': 190,
      'followers': '700k followers'
    },
    {
      'imagepath': ImageConstatnts.image_11,
      'description': "Nature's palette is the most beautiful.",
      'profile_pic_url': ImageConstatnts.profile_pic10,
      'name': 'Mia',
      'likeCount': 1099,
      'followers': '1.3m followers'
    },
    {
      'imagepath': ImageConstatnts.image_12,
      'description': "Adventure awaits in every corner of nature.",
      'profile_pic_url': ImageConstatnts.profile_pic11,
      'name': 'James',
      'likeCount': 155,
      'followers': '1m followers'
    },
    {
      'imagepath': ImageConstatnts.image_13,
      'description': "Serenity found in nature's embrace.",
      'profile_pic_url': ImageConstatnts.profile_pic12,
      'name': 'Chloe',
      'likeCount': 1090,
      'followers': '550k followers'
    },
    {
      'imagepath': ImageConstatnts.image_14,
      'description': "The earth has music for those who listen.",
      'profile_pic_url': ImageConstatnts.profile_pic13,
      'name': 'Ethan',
      'likeCount': 776,
      'followers': '1.7m followers'
    },
    {
      'imagepath': ImageConstatnts.image_15,
      'description': "Exploring the wild, finding my soul.",
      'profile_pic_url': ImageConstatnts.profile_pic14,
      'name': 'Hana',
      'likeCount': 187,
      'followers': '650k followers'
    },
    {
      'imagepath': ImageConstatnts.image_16,
      'description': "Nature's beauty never fades.",
      'profile_pic_url': ImageConstatnts.profile_pic15,
      'name': 'rahul',
      'likeCount': 178,
      'followers': '690k followers'
    },
  ];

  //search page container pic
  static List<Map> pageview_data = [
    {
      'url': ImageConstatnts.Craft1,
      'profile': 'Clay Designer',
      'title': "The life so short, the craft so long to learn."
    },
    {
      'url': ImageConstatnts.Craft2,
      'profile': 'Bags for You',
      'title': "Craft is the vehicle for expressing your vision.."
    },
    {
      'url': ImageConstatnts.Craft3,
      'profile': 'Painting Ideas',
      'title': "Art is a way to make your soul grow. So do it."
    },
    {
      'url': ImageConstatnts.Craft4,
      'profile': 'Clay Art',
      'title': "Craftsmanship names an enduring"
    },
    {
      'url': ImageConstatnts.Craft5,
      'profile': 'John Daniel',
      'title': "Creativity is allowing yourself to make mistakes"
    },
    {
      'url': ImageConstatnts.Craft6,
      'profile': 'Mtathew Creator',
      'title': " Craft is the visible edge of art"
    },
  ];
  //search page ideas pic
  static List<String> idea_pics_url = [
    'https://images.pexels.com/photos/4041283/pexels-photo-4041283.jpeg?auto=compress&cs=tinysrgb&w=400',
    'https://images.pexels.com/photos/9999606/pexels-photo-9999606.jpeg?auto=compress&cs=tinysrgb&w=400',
    'https://i.pinimg.com/474x/e1/0a/c4/e10ac4122f4486d51eb07bce328c2a2e.jpg',
    'https://i.pinimg.com/736x/25/2a/d3/252ad3c7386b135c59c1c878e2f617ee.jpg',
    'https://i.pinimg.com/474x/db/5b/61/db5b6119a0ab67a7c4ff1d4b149fd4c9.jpg',
    'https://i.pinimg.com/474x/4b/c1/47/4bc147bb1e2d76dae32bcddbd2be52a7.jpg'
  ];
  static List<String> profile_pic = [
    'https://images.pexels.com/photos/3771079/pexels-photo-3771079.jpeg?auto=compress&cs=tinysrgb&w=400',
    'https://images.pexels.com/photos/3769013/pexels-photo-3769013.jpeg?auto=compress&cs=tinysrgb&w=400',
    'https://images.pexels.com/photos/7622882/pexels-photo-7622882.jpeg?auto=compress&cs=tinysrgb&w=400',
    'https://images.pexels.com/photos/16313940/pexels-photo-16313940/free-photo-of-woman-in-blue-jeans-sitting-and-painting.jpeg?auto=compress&cs=tinysrgb&w=400',
    'https://images.pexels.com/photos/9724743/pexels-photo-9724743.jpeg?auto=compress&cs=tinysrgb&w=400',
    'https://images.pexels.com/photos/4350172/pexels-photo-4350172.jpeg?auto=compress&cs=tinysrgb&w=400'
  ];
  static List<Map> ideas = [
    {
      'url':
          'https://i.pinimg.com/474x/10/45/eb/1045eb8fa0215e3730496f074de758c0.jpg',
      'title': 'Crochet design pattern'
    },
    {
      'url':
          'https://i.pinimg.com/474x/4d/40/45/4d40457efe8c8d34f3ac00a6575d767b.jpg',
      "title": 'Couple hidden face pics'
    },
    {
      'url':
          'https://i.pinimg.com/564x/08/ef/4c/08ef4c165455ff2ec12c0f46e7fb19de.jpg',
      'title': 'Ruby jewelry'
    },
    {
      'url':
          'https://i.pinimg.com/474x/c2/50/9b/c2509bf08c2a6d9bbabb88a3cac7e158.jpg',
      'title': 'Paper Art'
    }
  ];
  //  {
  //     'url':
  //     'title':
  //   }

  static List<Map> popularIdeas = [
    {
      'url':
          'https://i.pinimg.com/474x/be/aa/86/beaa86edaec8800144e95adf66f496f8.jpg',
      'title': 'Designing Ideas'
    },
    {
      'url':
          'https://i.pinimg.com/474x/42/61/b2/4261b2ce5ac49a892c09a36a38c31a97.jpg',
      'title': 'Saree ideas'
    },
    {
      'url':
          'https://i.pinimg.com/474x/24/17/9a/24179a9c2750835b9468b37864b5dc89.jpg',
      'title': 'Gifts'
    },
    {
      'url':
          'https://i.pinimg.com/474x/d8/9e/e4/d89ee49e9f3ca55a79fbc7a053321200.jpg',
      'title': 'Wall paper'
    },
    {
      'url':
          'https://i.pinimg.com/564x/90/5a/95/905a9548fa340997856f4025f9f75580.jpg',
      'title': 'Tattoos'
    },
    {
      'url':
          'https://i.pinimg.com/474x/13/b6/d5/13b6d5001f83df8fee20553affb91370.jpg',
      'title': 'Simple Jewely'
    },
  ];

  static List<Map> InboxData = [
    {
      'url':
          'https://images.pexels.com/photos/415829/pexels-photo-415829.jpeg?auto=compress&cs=tinysrgb&w=600',
      'name': 'Ally',
    },
    {
      'url':
          'https://images.pexels.com/photos/1674752/pexels-photo-1674752.jpeg?auto=compress&cs=tinysrgb&w=600',
      'name': 'Deepika',
    },
    {
      'url':
          'https://images.pexels.com/photos/771742/pexels-photo-771742.jpeg?auto=compress&cs=tinysrgb&w=600',
      'name': 'EOLIN',
    },
    {
      'url':
          'https://images.pexels.com/photos/1043471/pexels-photo-1043471.jpeg?auto=compress&cs=tinysrgb&w=600',
      'name': 'Razak',
    },
    {
      'url':
          'https://images.pexels.com/photos/1499327/pexels-photo-1499327.jpeg?auto=compress&cs=tinysrgb&w=600',
      'name': 'Sam',
    },
    {
      'url':
          'https://images.pexels.com/photos/1559486/pexels-photo-1559486.jpeg?auto=compress&cs=tinysrgb&w=600',
      'name': 'Tommy',
    },
    {
      'url':
          'https://images.pexels.com/photos/1080213/pexels-photo-1080213.jpeg?auto=compress&cs=tinysrgb&w=600',
      'name': 'Krish',
    }
  ];
  //chat tab-updates screen
  static List<Map> stillSearchingData = [
    {'topic': 'Rain', 'week': '3d'},
    {'topic': 'Beauty', 'week': '4d'},
    {'topic': 'Onam Look', 'week': '1w'},
    {'topic': 'Nature', 'week': '2w'}
  ];
  static List<Map> rowCardData = [
    {
      'imageurl': ImageConstatnts.Craft2,
      'title': 'Pins inspired by you',
      'week': '2w'
    },
    {
      'imageurl': ImageConstatnts.Craft1,
      'title': 'your Home feed has new Pins',
      'week': '3w'
    },
    // {'imageurl': '', 'title': '', 'week': ''},
  ];
  static List<Map> moreIdeasData = [
    {
      'imageurl': ImageConstatnts.moreideas1,
      'title': 'Matching Tattoo for You',
      'week': '1mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas2,
      'title': 'Ideas for you',
      'week': '2mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas3,
      'title': 'Tattoo for you',
      'week': '2mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas4,
      'title': 'Pins inspired by You',
      'week': '3mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas5,
      'title': 'Tattoos and More',
      'week': '3mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas6,
      'title': 'Your Home feed has new pins',
      'week': '3mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas7,
      'title': 'Indian Outfit and more',
      'week': '4mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas8,
      'title': 'Here are 8 boards with ideas you might like',
      'week': '4mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas9,
      'title': 'Pins inspired by You',
      'week': '4mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas10,
      'title': 'we think you might like these 14 ideas',
      'week': '5mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas11,
      'title': 'Here are 10 boards with ideas you might like',
      'week': '5mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas12,
      'title': 'we think you might likeb these 18 ideas',
      'week': '5mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas13,
      'title': 'Dress and more',
      'week': '5mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas14,
      'title': 'Your Home feed has new pins',
      'week': '6mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas15,
      'title': 'Tattoos and More',
      'week': '6mo'
    },
    {
      'imageurl': ImageConstatnts.moreideas16,
      'title': 'Pins inspired by You',
      'week': '6mo'
    },
  ];
  // profile page container data
  static List<Map> profilePageData = [
    {
      'url': ImageConstatnts.profilePage1,
      'title': 'Simple Rings',
      'number': '15 Pins'
    },
    {
      'url': ImageConstatnts.profilePage2,
      'title': 'Traditional Wears',
      'number': '14 Pins'
    },
    {
      'url': ImageConstatnts.profilePage3,
      'title': 'Blouse Designs',
      'number': '14 Pins'
    },
    {
      'url': ImageConstatnts.profilePage4,
      'title': 'Home Decor',
      'number': '17 Pins'
    },
  ];
  // caroulsel screens detailng page details of search screen
  static List<Map> carouselDetailingData = [
    {
      'title': 'clay art',
      'caption': [
        'Cute meow pots',
        '',
        'How is this Scenary?',
        'Coffeee?',
        '',
        'Beautiful..'
      ],
      'description':
          'From humble clay, creation springs anew,In hands of artists, life and form pursue.With every turn, a masterpiece unfolds,In earthen whispers, timeless art is told.',
      'image_url': [
        'https://i.pinimg.com/474x/16/fd/fc/16fdfcf26ffc47c834a1f9c2dbed63a6.jpg',
        'https://i.pinimg.com/474x/0b/10/a8/0b10a8cda48d921fcf1d3352a8c1218d.jpg',
        'https://i.pinimg.com/474x/70/5b/96/705b96cc05f8ff4400daef3d3e36ea18.jpg',
        'https://i.pinimg.com/474x/d0/63/26/d06326365935dee9dfbf06e6381b2283.jpg',
        'https://i.pinimg.com/474x/1e/5c/87/1e5c87d7c02f102a56863725e6884672.jpg',
        'https://i.pinimg.com/474x/f8/ca/0f/f8ca0f3409b99d44829408ee62559222.jpg'
      ],
      'profile_pic': [
        'https://images.pexels.com/photos/8159657/pexels-photo-8159657.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=170.625&fit=crop&h=276.25',
        'https://images.pexels.com/photos/10757699/pexels-photo-10757699.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/24235269/pexels-photo-24235269/free-photo-of-photo-of-a-young-woman-holding-her-hair-up.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/24260386/pexels-photo-24260386/free-photo-of-black-and-white-photo-of-a-young-man-standing-outside.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/24185772/pexels-photo-24185772/free-photo-of-portrait-of-brunette-woman-in-a-studio.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/24181535/pexels-photo-24181535/free-photo-of-man-in-shirt-and-patterned-waistcoat.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profile_name': ['Oliver', 'Emma', 'Liam', 'Sophia', 'Noah', 'Ava'],
      'followers': [
        '90 followers',
        '23 followers',
        '250 followers',
        '467 followers',
        '234 followers',
        '9876 followers',
      ],
      'D_description': [
        "Clay art is a versatile medium.",
        "It allows for creative expression.",
        "You can mold it into any shape.",
        "It is often used in pottery.",
        "Clay art can be painted.",
        "It requires patience and skill."
      ],
    },
    {
      'title': 'Craft Bags',
      'caption': [
        'Cute ?',
        'How is this',
        'Classic',
        'Simple..',
        'PineApple..',
        '',
      ],
      'description':
          "Within the bag, a world of dreams unfurls,Threads and tools, like hidden pearls.With every stitch, and careful seam,A maker's heart in crafted dreams.",
      'image_url': [
        'https://i.pinimg.com/474x/15/70/8b/15708b056749dcc140adbf4cb2871e6d.jpg',
        'https://i.pinimg.com/474x/b6/10/bc/b610bcf902a9daea0ac923c73f13069a.jpg',
        'https://i.pinimg.com/474x/7c/14/13/7c1413ff0e224bf00c32bcef85e1f9f6.jpg',
        'https://i.pinimg.com/474x/63/18/67/631867b966c6e4f414e2b5da8c38204d.jpg',
        'https://i.pinimg.com/474x/3a/f5/4e/3af54ef92fcf957dab8e2270b439230f.jpg',
        'https://i.pinimg.com/474x/24/f0/6d/24f06dece4bac1196028b6013d53c523.jpg'
      ],
      'profile_pic': [
        'https://images.pexels.com/photos/23995223/pexels-photo-23995223/free-photo-of-attractive-woman-sitting-on-a-chair-against-white-background.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/23931918/pexels-photo-23931918/free-photo-of-closeup-of-brunette-person-face-with-smoke.png?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/23945347/pexels-photo-23945347/free-photo-of-portrait-of-woman-wearing-a-hat.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/23939543/pexels-photo-23939543/free-photo-of-side-view-of-young-woman-fixing-her-hair.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1858175/pexels-photo-1858175.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/774909/pexels-photo-774909.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profile_name': [
        'Oliver',
        'Emma',
        'Liam',
        'Sophia',
        'Noah',
        'Ava',
        'Ethan',
      ],
      'followers': [
        '123 followers',
        '7654 followers',
        '765 followers',
        '123 followers',
        '987 followers',
        '23 followers',
      ],
      'D_description': [
        "Craft bags are perfect for DIY projects.",
        "They come in various sizes and colors.",
        "You can decorate them with paint.",
        "They are great for gift wrapping.",
        "Craft bags are reusable.",
        "They are made from different materials."
      ],
    },
    {
      'title': 'Painting Ideas',
      'caption': ['', 'Cuties', '', 'My Painting', '', 'Beautiful..'],
      'description':
          "Brush in hand, a canvas wide,Colors dance and worlds collide.In every stroke, a story blooms,A painter’s soul, in hues and plumes.",
      'image_url': [
        'https://i.pinimg.com/474x/4f/78/a2/4f78a2ac80a385cc2c40e2d633439df7.jpg',
        'https://i.pinimg.com/474x/57/39/fc/5739fc0fde77a7f1e90fc93aa252c035.jpg',
        'https://i.pinimg.com/474x/64/bb/e3/64bbe36b1c2d5222f5294ab21d87b964.jpg',
        'https://i.pinimg.com/474x/5d/1b/3e/5d1b3e2ea9556a73cac40493655fe647.jpg',
        'https://i.pinimg.com/474x/3f/d3/9f/3fd39fdfec2955e40498061cdb500f3b.jpg',
        'https://i.pinimg.com/474x/47/aa/95/47aa95a870691f2d0aa90f1ba95748b8.jpg'
      ],
      'profile_pic': [
        'https://images.pexels.com/photos/1987301/pexels-photo-1987301.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/22814807/pexels-photo-22814807/free-photo-of-blonde-woman-posing-in-pink-dress.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/3094215/pexels-photo-3094215.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/2709388/pexels-photo-2709388.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/2379004/pexels-photo-2379004.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1572878/pexels-photo-1572878.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profile_name': [
        'Isabella',
        'Mason',
        'Mia',
        'Lucas',
        'Amelia',
        'Logan',
      ],
      'followers': [
        '765 followers',
        '34 followers',
        '98 followers',
        '23 followers',
        '98 followers',
        '567 followers',
      ],
      'D_description': [
        "Paintings capture emotions and stories.",
        "They come in various styles and techniques.",
        "Colors play a significant role in paintings.",
        "Abstract art can be open to interpretation.",
        "Portraits focus on capturing likeness.",
        "Landscapes often depict natural beauty."
      ],
    },
    {
      'title': 'Cute Clay Arts',
      'caption': [
        'Cuties',
        '',
        'How is this ?',
        'Little flowers',
        '',
        '',
        'Beautiful..',
        'What about me?'
      ],
      'description':
          " Art takes its form,In sculptor's hands, creations warm.Shapes and dreams in mud arise,Crafting beauty beneath the skies.",
      'image_url': [
        'https://i.pinimg.com/474x/17/52/28/17522874f1c3324cc46d67a6b79ec668.jpg',
        'https://i.pinimg.com/474x/25/00/e4/2500e4f8314284ced3dc21f882e45e67.jpg',
        'https://i.pinimg.com/474x/c1/4b/18/c14b18852e9a55600bf2d1ca8d8243ec.jpg',
        'https://i.pinimg.com/474x/a9/2d/17/a92d175e14ca94712cc373e7d42320ab.jpg',
        'https://i.pinimg.com/474x/a4/39/eb/a439eb82a0baf8a29340e68bd30e72ca.jpg',
        'https://i.pinimg.com/474x/0f/07/46/0f0746defdaf22aafe6a3d32ac7a6af3.jpg',
        'https://i.pinimg.com/474x/25/67/c7/2567c79f3e5c7a2df6c331e071001d3d.jpg',
        'https://i.pinimg.com/474x/1e/3d/3d/1e3d3daf28773b7547f40368bd3c7cb8.jpg'
      ],
      'profile_pic': [
        'https://images.pexels.com/photos/2065195/pexels-photo-2065195.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/26584711/pexels-photo-26584711/free-photo-of-portrait-of-woman-on-a-balcony-at-night.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/771742/pexels-photo-771742.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1898555/pexels-photo-1898555.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/733872/pexels-photo-733872.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/13069698/pexels-photo-13069698.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1130626/pexels-photo-1130626.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1821095/pexels-photo-1821095.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profile_name': [
        'Charlotte',
        'Elijah',
        'Harper',
        'James',
        'Evelyn',
        'Benjamin',
        'Sophia',
        'Noah',
      ],
      'followers': [
        '765 followers',
        '2345 followers',
        '98 followers',
        '345 followers',
        '234 followers',
        '098 followers',
        '3456 followers',
        '98 followers',
      ],
      'D_description': [
        "Cute clay art includes mini sculptures.",
        "Tiny animals are popular choices.",
        "Bright colors make them appealing.",
        "They are often used as charms.",
        "Kids love making cute clay art.",
        "It can be turned into jewelry.",
        "Cute clay art makes great gifts.",
        "Simple designs are often the best."
      ],
    },
    {
      'title': 'Cute Painting Ideas',
      'caption': ['', '', 'Fishy', '', 'Beautiful..'],
      'description':
          "With every brushstroke, visions flow,Colors blend, emotions show.On canvas wide, dreams take flight,A painter's heart in hues of light.",
      'image_url': [
        'https://i.pinimg.com/474x/27/48/e1/2748e1f6c32045729d737314c0e464db.jpg',
        'https://i.pinimg.com/474x/21/16/5a/21165a0aef64552dd65ed8e9c7c83b17.jpg',
        'https://i.pinimg.com/474x/4f/d9/ef/4fd9ef6497e4d0a175fcebe74315951b.jpg',
        'https://i.pinimg.com/474x/5b/63/54/5b63547ab4ec389ccd8bc3a9358dadaa.jpg',
        'https://i.pinimg.com/474x/49/ac/d3/49acd3d17d2be8c2d1ba1db9797e13b4.jpg'
      ],
      'profile_pic': [
        'https://images.pexels.com/photos/718978/pexels-photo-718978.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/12244376/pexels-photo-12244376.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/4029925/pexels-photo-4029925.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/810775/pexels-photo-810775.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/810775/pexels-photo-810775.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profile_name': [
        'Jacob',
        'Emily',
        'Alexander',
        'Scarlett',
        'Daniel',
      ],
      'followers': [
        '456 followers',
        '65 followers',
        '876 followers',
        '987 followers',
        '123 followers',
      ],
      'D_description': [
        "Cute painting art features adorable characters.",
        "Bright colors enhance the charm.",
        "Simple designs are easy to create.",
        "Animals and cartoon styles are popular.",
        "Cute art makes great decorations."
      ],
    },
    {
      'title': 'Indoor Plants ',
      'caption': [
        'Elegant',
        '',
        'How is this ?',
        '',
        '',
        'Beautiful..',
        '',
        '',
        'cute',
        '',
        'My Fav',
        'Rooms like this'
      ],
      'description':
          "Within these walls, a world unfolds,Quiet corners, stories told.In cozy nooks, life's moments gleam,Indoors, we weave our sweetest dreams.",
      'image_url': [
        'https://i.pinimg.com/474x/23/d8/19/23d8194d395c0385acc9a60558e2130d.jpg',
        'https://i.pinimg.com/474x/9f/be/0c/9fbe0cd6c1335ad0a3a3a9d1aed03ef3.jpg',
        'https://i.pinimg.com/474x/70/3e/9c/703e9c1d17c60c3343d8495957d5dd74.jpg',
        'https://i.pinimg.com/474x/9d/e8/2f/9de82f08e7b79c919d3f1a9a40b39768.jpg',
        'https://i.pinimg.com/474x/e8/e1/7a/e8e17aecbb6125a0a3491ad961827b2a.jpg',
        'https://i.pinimg.com/474x/76/a3/fd/76a3fd47e3cdcf17b60bb56fbdb8d420.jpg',
        'https://i.pinimg.com/474x/aa/6f/c4/aa6fc424222d40d4d0deddf7d53cee8d.jpg',
        'https://i.pinimg.com/474x/7a/25/59/7a255972445ab83290a60e1f29a9f236.jpg'
      ],
      'profile_pic': [
        'https://images.pexels.com/photos/1559193/pexels-photo-1559193.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/7173047/pexels-photo-7173047.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1036620/pexels-photo-1036620.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/4946515/pexels-photo-4946515.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1441151/pexels-photo-1441151.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/3778361/pexels-photo-3778361.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/3866555/pexels-photo-3866555.png?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1587009/pexels-photo-1587009.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profile_name': [
        'Madison',
        'Henry',
        'Aria',
        'Matthew',
        'Grace',
        'Harper',
        'James',
      ],
      'followers': [
        '234 followers',
        '65 followers',
        '987 followers',
        '5 followers',
        '987 followers',
        '23 followers',
        '8 followers',
        '234 followers',
      ],
      'D_description': [
        "Indoor plants improve air quality.",
        "They add a touch of nature to your home.",
        "Succulents are easy to care for.",
        "Ferns thrive in low light.",
        "Watering needs vary by plant type.",
        "Indoor plants can boost your mood.",
        "Some plants need misting.",
        "Plants can be grown in various containers."
      ],
    },
  ];
  static List crouselUtraDetailing_clay = [
    {
      'image_url': [
        'https://i.pinimg.com/474x/16/fd/fc/16fdfcf26ffc47c834a1f9c2dbed63a6.jpg',
        'https://i.pinimg.com/474x/0b/10/a8/0b10a8cda48d921fcf1d3352a8c1218d.jpg',
        'https://i.pinimg.com/474x/70/5b/96/705b96cc05f8ff4400daef3d3e36ea18.jpg',
        'https://i.pinimg.com/474x/d0/63/26/d06326365935dee9dfbf06e6381b2283.jpg',
        'https://i.pinimg.com/474x/1e/5c/87/1e5c87d7c02f102a56863725e6884672.jpg',
        'https://i.pinimg.com/474x/f8/ca/0f/f8ca0f3409b99d44829408ee62559222.jpg'
      ],
      'profilePic': [
        'https://images.pexels.com/photos/8159657/pexels-photo-8159657.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=170.625&fit=crop&h=276.25',
        'https://images.pexels.com/photos/10757699/pexels-photo-10757699.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/24235269/pexels-photo-24235269/free-photo-of-photo-of-a-young-woman-holding-her-hair-up.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/24260386/pexels-photo-24260386/free-photo-of-black-and-white-photo-of-a-young-man-standing-outside.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/24185772/pexels-photo-24185772/free-photo-of-portrait-of-brunette-woman-in-a-studio.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/24181535/pexels-photo-24181535/free-photo-of-man-in-shirt-and-patterned-waistcoat.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profileName': ['Oliver', 'Emma', 'Liam', 'Sophia', 'Noah', 'Ava'],
      'followers': [
        '90 followers',
        '23 followers',
        '250 followers',
        '467 followers',
        '234 followers',
        '9876 followers',
      ],
      'd_description': [
        "Clay art is a versatile medium.",
        "It allows for creative expression.",
        "You can mold it into any shape.",
        "It is often used in pottery.",
        "Clay art can be painted.",
        "It requires patience and skill."
      ],
    },
  ];

  static List crouselUtraDetailing_bags = [
    {
      'image_url': [
        'https://i.pinimg.com/474x/15/70/8b/15708b056749dcc140adbf4cb2871e6d.jpg',
        'https://i.pinimg.com/474x/b6/10/bc/b610bcf902a9daea0ac923c73f13069a.jpg',
        'https://i.pinimg.com/474x/7c/14/13/7c1413ff0e224bf00c32bcef85e1f9f6.jpg',
        'https://i.pinimg.com/474x/63/18/67/631867b966c6e4f414e2b5da8c38204d.jpg',
        'https://i.pinimg.com/474x/3a/f5/4e/3af54ef92fcf957dab8e2270b439230f.jpg',
        'https://i.pinimg.com/474x/24/f0/6d/24f06dece4bac1196028b6013d53c523.jpg'
      ],
      'profilePic': [
        'https://images.pexels.com/photos/23995223/pexels-photo-23995223/free-photo-of-attractive-woman-sitting-on-a-chair-against-white-background.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/23931918/pexels-photo-23931918/free-photo-of-closeup-of-brunette-person-face-with-smoke.png?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/23945347/pexels-photo-23945347/free-photo-of-portrait-of-woman-wearing-a-hat.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/23939543/pexels-photo-23939543/free-photo-of-side-view-of-young-woman-fixing-her-hair.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1858175/pexels-photo-1858175.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/774909/pexels-photo-774909.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profileName': [
        'Oliver',
        'Emma',
        'Liam',
        'Sophia',
        'Noah',
        'Ava',
        'Ethan',
      ],
      'followers': [
        '123 followers',
        '7654 followers',
        '765 followers',
        '123 followers',
        '987 followers',
        '23 followers',
      ],
      'd_description': [
        "Craft bags are perfect for DIY projects.",
        "They come in various sizes and colors.",
        "You can decorate them with paint.",
        "They are great for gift wrapping.",
        "Craft bags are reusable.",
        "They are made from different materials."
      ],
    },
  ];
  static List crouselUtraDetailing_painting = [
    {
      'image_url': [
        'https://i.pinimg.com/474x/4f/78/a2/4f78a2ac80a385cc2c40e2d633439df7.jpg',
        'https://i.pinimg.com/474x/57/39/fc/5739fc0fde77a7f1e90fc93aa252c035.jpg',
        'https://i.pinimg.com/474x/64/bb/e3/64bbe36b1c2d5222f5294ab21d87b964.jpg',
        'https://i.pinimg.com/474x/5d/1b/3e/5d1b3e2ea9556a73cac40493655fe647.jpg',
        'https://i.pinimg.com/474x/3f/d3/9f/3fd39fdfec2955e40498061cdb500f3b.jpg',
        'https://i.pinimg.com/474x/47/aa/95/47aa95a870691f2d0aa90f1ba95748b8.jpg'
      ],
      'profilePic': [
        'https://images.pexels.com/photos/1987301/pexels-photo-1987301.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/22814807/pexels-photo-22814807/free-photo-of-blonde-woman-posing-in-pink-dress.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/3094215/pexels-photo-3094215.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/2709388/pexels-photo-2709388.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/2379004/pexels-photo-2379004.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1572878/pexels-photo-1572878.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profileName': [
        'Isabella',
        'Mason',
        'Mia',
        'Lucas',
        'Amelia',
        'Logan',
      ],
      'followers': [
        '765 followers',
        '34 followers',
        '98 followers',
        '23 followers',
        '98 followers',
        '567 followers',
      ],
      'd_description': [
        "Paintings capture emotions and stories.",
        "They come in various styles and techniques.",
        "Colors play a significant role in paintings.",
        "Abstract art can be open to interpretation.",
        "Portraits focus on capturing likeness.",
        "Landscapes often depict natural beauty."
      ],
    },
  ];
  static List crouselUtraDetailing_cutePainting = [
    {
      'image_url': [
        'https://i.pinimg.com/474x/27/48/e1/2748e1f6c32045729d737314c0e464db.jpg',
        'https://i.pinimg.com/474x/21/16/5a/21165a0aef64552dd65ed8e9c7c83b17.jpg',
        'https://i.pinimg.com/474x/4f/d9/ef/4fd9ef6497e4d0a175fcebe74315951b.jpg',
        'https://i.pinimg.com/474x/5b/63/54/5b63547ab4ec389ccd8bc3a9358dadaa.jpg',
        'https://i.pinimg.com/474x/49/ac/d3/49acd3d17d2be8c2d1ba1db9797e13b4.jpg'
      ],
      'profilePic': [
        'https://images.pexels.com/photos/718978/pexels-photo-718978.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/12244376/pexels-photo-12244376.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/4029925/pexels-photo-4029925.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/810775/pexels-photo-810775.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/810775/pexels-photo-810775.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profileName': [
        'Jacob',
        'Emily',
        'Alexander',
        'Scarlett',
        'Daniel',
      ],
      'followers': [
        '456 followers',
        '65 followers',
        '876 followers',
        '987 followers',
        '123 followers',
      ],
      'd_description': [
        "Cute painting art features adorable characters.",
        "Bright colors enhance the charm.",
        "Simple designs are easy to create.",
        "Animals and cartoon styles are popular.",
        "Cute art makes great decorations."
      ],
    },
  ];
  static List crouselUtraDetailing_cuteClay = [
    {
      'image_url': [
        'https://i.pinimg.com/474x/17/52/28/17522874f1c3324cc46d67a6b79ec668.jpg',
        'https://i.pinimg.com/474x/25/00/e4/2500e4f8314284ced3dc21f882e45e67.jpg',
        'https://i.pinimg.com/474x/c1/4b/18/c14b18852e9a55600bf2d1ca8d8243ec.jpg',
        'https://i.pinimg.com/474x/a9/2d/17/a92d175e14ca94712cc373e7d42320ab.jpg',
        'https://i.pinimg.com/474x/a4/39/eb/a439eb82a0baf8a29340e68bd30e72ca.jpg',
        'https://i.pinimg.com/474x/0f/07/46/0f0746defdaf22aafe6a3d32ac7a6af3.jpg',
        'https://i.pinimg.com/474x/25/67/c7/2567c79f3e5c7a2df6c331e071001d3d.jpg',
        'https://i.pinimg.com/474x/1e/3d/3d/1e3d3daf28773b7547f40368bd3c7cb8.jpg'
      ],
      'profilePic': [
        'https://images.pexels.com/photos/2065195/pexels-photo-2065195.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/26584711/pexels-photo-26584711/free-photo-of-portrait-of-woman-on-a-balcony-at-night.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/771742/pexels-photo-771742.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1898555/pexels-photo-1898555.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/733872/pexels-photo-733872.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/13069698/pexels-photo-13069698.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1130626/pexels-photo-1130626.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1821095/pexels-photo-1821095.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profileName': [
        'Charlotte',
        'Elijah',
        'Harper',
        'James',
        'Evelyn',
        'Benjamin',
        'Sophia',
        'Noah',
      ],
      'followers': [
        '765 followers',
        '2345 followers',
        '98 followers',
        '345 followers',
        '234 followers',
        '098 followers',
        '3456 followers',
        '98 followers',
      ],
      'd_description': [
        "Cute clay art includes mini sculptures.",
        "Tiny animals are popular choices.",
        "Bright colors make them appealing.",
        "They are often used as charms.",
        "Kids love making cute clay art.",
        "It can be turned into jewelry.",
        "Cute clay art makes great gifts.",
        "Simple designs are often the best."
      ],
    },
  ];
  static List crouselUtraDetailing_indoorPlants = [
    {
      'image_url': [
        'https://i.pinimg.com/474x/23/d8/19/23d8194d395c0385acc9a60558e2130d.jpg',
        'https://i.pinimg.com/474x/9f/be/0c/9fbe0cd6c1335ad0a3a3a9d1aed03ef3.jpg',
        'https://i.pinimg.com/474x/70/3e/9c/703e9c1d17c60c3343d8495957d5dd74.jpg',
        'https://i.pinimg.com/474x/9d/e8/2f/9de82f08e7b79c919d3f1a9a40b39768.jpg',
        'https://i.pinimg.com/474x/e8/e1/7a/e8e17aecbb6125a0a3491ad961827b2a.jpg',
        'https://i.pinimg.com/474x/76/a3/fd/76a3fd47e3cdcf17b60bb56fbdb8d420.jpg',
        'https://i.pinimg.com/474x/aa/6f/c4/aa6fc424222d40d4d0deddf7d53cee8d.jpg',
        'https://i.pinimg.com/474x/7a/25/59/7a255972445ab83290a60e1f29a9f236.jpg'
      ],
      'profilePic': [
        'https://images.pexels.com/photos/1559193/pexels-photo-1559193.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/7173047/pexels-photo-7173047.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1036620/pexels-photo-1036620.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/4946515/pexels-photo-4946515.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1441151/pexels-photo-1441151.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/3778361/pexels-photo-3778361.jpeg?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/3866555/pexels-photo-3866555.png?auto=compress&cs=tinysrgb&w=400',
        'https://images.pexels.com/photos/1587009/pexels-photo-1587009.jpeg?auto=compress&cs=tinysrgb&w=400',
      ],
      'profileName': [
        'Madison',
        'Henry',
        'Aria',
        'Matthew',
        'Grace',
        'Harper',
        'James',
      ],
      'followers': [
        '234 followers',
        '65 followers',
        '987 followers',
        '5 followers',
        '987 followers',
        '23 followers',
        '8 followers',
        '234 followers',
      ],
      'd_description': [
        "Indoor plants improve air quality.",
        "They add a touch of nature to your home.",
        "Succulents are easy to care for.",
        "Ferns thrive in low light.",
        "Watering needs vary by plant type.",
        "Indoor plants can boost your mood.",
        "Some plants need misting.",
        "Plants can be grown in various containers."
      ],
    },
  ];
  static List pinnedPics = [
    'https://i.pinimg.com/564x/32/cb/2f/32cb2f18229be0dbc71e5c4331f13908.jpg',
    'https://i.pinimg.com/474x/fd/35/d2/fd35d24c172c08a271888a9fbead2ddc.jpg',
    'https://i.pinimg.com/474x/84/c1/ac/84c1acf4fc5d7631f2feb38474a09d41.jpg',
    'https://i.pinimg.com/474x/b3/ad/bb/b3adbbbc402299b3604b20675b91600e.jpg',
    'https://i.pinimg.com/474x/01/e2/c4/01e2c4baea205c6c3729c595b15828c5.jpg',
    'https://i.pinimg.com/474x/0c/e7/40/0ce7402a60b426c8ce8ab536753ee402.jpg',
    'https://i.pinimg.com/474x/99/ab/56/99ab56d07bdc0ac8485a8ddadd732f82.jpg',
    'https://i.pinimg.com/474x/6a/2a/50/6a2a50b1791372027313b241b327ff5d.jpg',
    'https://i.pinimg.com/474x/b1/99/76/b199762f6e64a708a5f58eac07325119.jpg',
    'https://i.pinimg.com/474x/9d/ef/21/9def2143bd88ddaf8d81efdb50cd5edb.jpg',
    'https://i.pinimg.com/474x/ab/bc/a6/abbca671f3c6c565e25a42feb2c02ee8.jpg',
    'https://i.pinimg.com/474x/b8/68/ca/b868ca2cb45de0b5c4634428eae91d61.jpg',
    'https://i.pinimg.com/474x/4b/80/f7/4b80f74d6257fcece2af4bd941944e94.jpg',
    'https://i.pinimg.com/474x/5e/b5/38/5eb5381eddf4b8af7ad8dd29a85507a8.jpg',
    'https://i.pinimg.com/474x/bd/f1/f3/bdf1f363fda33b2b04649f174994526a.jpg',
    'https://i.pinimg.com/474x/41/7f/a5/417fa5aa3c2e0214f1fb5d2c4146c0ff.jpg',
    'https://i.pinimg.com/474x/af/39/58/af3958fd4b41236640adc34bcf9e95d6.jpg',
    'https://i.pinimg.com/474x/18/bb/d1/18bbd12eab457e5a178d0425d3c98f58.jpg',
    'https://i.pinimg.com/474x/8d/de/b8/8ddeb826db17ef9b9ec43a170d984551.jpg',
    'https://i.pinimg.com/474x/3d/ea/16/3dea167970c32d79ecb8e77d2d0f0325.jpg',
    'https://i.pinimg.com/474x/9e/80/2b/9e802ba52fd56e213657c58a01f4b4fb.jpg',
    'https://i.pinimg.com/474x/c5/8a/97/c58a97626d2ca5b132c3486bdc0e6b9e.jpg',
    'https://i.pinimg.com/474x/04/2a/30/042a30dc836ce2b246f5105bad18047c.jpg',
    'https://i.pinimg.com/474x/0a/0f/dd/0a0fddff75a5fb65f7fb5e4594042c40.jpg',
    'https://i.pinimg.com/474x/d0/86/fd/d086fd5b2bd586fe6405bbc25a2fd4c7.jpg',
    'https://i.pinimg.com/474x/65/64/15/656415f8fbe62334aedaf498dbe17e3c.jpg',
    'https://i.pinimg.com/474x/28/e8/04/28e8047318c674812952d95c92f64c1a.jpg',
    'https://i.pinimg.com/474x/ba/17/37/ba1737563af88bf7d5d912157f5f125c.jpg',
    'https://i.pinimg.com/474x/20/03/cb/2003cb788c9420e9df07d96a7e711e98.jpg',
    'https://i.pinimg.com/474x/bf/5c/57/bf5c577e4a2a1e185973b4bb89a64088.jpg',
    'https://i.pinimg.com/474x/71/58/1c/71581c7f9fc253f5dd728a05fc940c80.jpg',
    'https://i.pinimg.com/474x/1c/99/d9/1c99d950182331f9fa585163f11a0875.jpg',
    'https://i.pinimg.com/474x/5e/7e/05/5e7e0533428b646fb7a403dd9ef5e450.jpg',
    'https://i.pinimg.com/474x/cc/e1/8a/cce18af87a0f6c9aec02362827e39b7a.jpg',
    'https://i.pinimg.com/474x/b0/d8/ff/b0d8ffc5018e52734df61f465f362eae.jpg',
    'https://i.pinimg.com/474x/3b/9d/96/3b9d96e06a6f716c58dba674f8094155.jpg',
    'https://i.pinimg.com/474x/5c/85/96/5c8596abba9403d4f2808d40caa4bcd5.jpg',
    'https://i.pinimg.com/474x/27/c5/c0/27c5c07fddf10de18575f8d69c58b5aa.jpg',
  ];
}

String? storegmail;
